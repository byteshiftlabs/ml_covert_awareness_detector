...
from config import (
    ...
    LOR_TIME, ROR_TIME, RUN2_TOTAL_TRS, TRANSITION_BUFFER, SPECIAL_SUBJECTS
)
...
def filter_by_motion(timeseries: pd.DataFrame, motion: pd.DataFrame) -> pd.DataFrame:
    """
...
def load_condition_3(subject: str) -> np.ndarray:
    ...
    From paper: concatenate run-2 (from lor+375 to end) + run-3 (from start to ror-375)
    ...
    if subject not in SPECIAL_SUBJECTS:
        ...
        motion_combined = pd.concat([motion2_lor, motion3_lor], ignore_index=True)
    else:
