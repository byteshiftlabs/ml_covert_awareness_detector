...
print(f"    Conscious (1):   {np.sum(y==1):3d} ({np.sum(y==1)/len(y)*100:.1f}%)")
print(f"    Unconscious (0): {np.sum(y==0):3d} ({np.sum(y==0)/len(y)*100:.1f}%)")
print(f"  Imbalance ratio: {np.sum(y==1)/np.sum(y==0):.1f}:1")
...
for i, test_subject in enumerate(unique_subjects):
    ...
    # Ensure we have enough samples for SMOTE (need at least 6 minority samples)
...
for threshold in np.arange(0.1, 0.9, 0.05):
    ...
    if bal_acc > best_balanced_acc:
        ...
        best_metrics = {
            ...
            'precision': precision_score(all_labels, y_pred_thresh, zero_division=0),
            'recall': recall_score(all_labels, y_pred_thresh, zero_division=0),
...
metrics_default = {
    ...
    'recall_unconscious': recall_score(all_labels, all_preds, pos_label=0, zero_division=0),
    'recall_conscious': recall_score(all_labels, all_preds, pos_label=1, zero_division=0),
...
print(f"Recall (Unconscious): {metrics_default['recall_unconscious']:.3f} ⚠️  (misses {(1-metrics_default['recall_unconscious'])*100:.0f}% unconscious)")
...
print(f"                 Predicted")
print(f"              Uncon  Consc")
print(f"Actual Uncon  {cm_default[0,0]:5d}  {cm_default[0,1]:5d}  ← Only {cm_default[0,0]}/{cm_default[0,0]+cm_default[0,1]} detected!")
print(f"       Consc  {cm_default[1,0]:5d}  {cm_default[1,1]:5d}")

print(f"\n2. OPTIMIZED THRESHOLD ({best_threshold:.2f})")
print("-" * 70)
print(f"Accuracy:          {best_metrics['accuracy']:.3f}")
print(f"Balanced Accuracy: {best_metrics['balanced_acc']:.3f} ✓ (Equal weight to both classes)")
print(f"Recall (Unconscious): {best_metrics['recall_unconscious']:.3f} {'✓' if best_metrics['recall_unconscious'] > 0.5 else '⚠️'}  (detects {best_metrics['recall_unconscious']*100:.0f}% unconscious)")
...
print(f"                 Predicted")
print(f"              Uncon  Consc")
print(f"Actual Uncon  {cm[0,0]:5d}  {cm[0,1]:5d}  ← {cm[0,0]}/{cm[0,0]+cm[0,1]} detected! (Improvement: +{cm[0,0]-cm_default[0,0]})")
print(f"       Consc  {cm[1,0]:5d}  {cm[1,1]:5d}")

print(f"\n{'='*70}")
print("IMPROVEMENT SUMMARY")
print('='*70)
print(f"Unconscious detection improved from {metrics_default['recall_unconscious']*100:.0f}% → {best_metrics['recall_unconscious']*100:.0f}%")
print(f"Balanced accuracy improved from {metrics_default['balanced_acc']:.3f} → {best_metrics['balanced_acc']:.3f}")
print(f"Trade-off: Overall accuracy {metrics_default['accuracy']:.3f} → {best_metrics['accuracy']:.3f}")
...
print(f"  • Will correctly identify {best_metrics['recall_unconscious']*100:.0f}% of unconscious patients")
print(f"  • Will correctly identify {best_metrics['recall_conscious']*100:.0f}% of conscious patients")
print(f"  • False alarm rate (conscious → unconscious): {(1-best_metrics['recall_conscious'])*100:.0f}%")
print(f"  • Miss rate (unconscious → conscious): {(1-best_metrics['recall_unconscious'])*100:.0f}%")
print(f"\nFor clinical use, prioritize:")
print(f"  - High recall on unconscious (minimize misses) ← Critical for safety")
print(f"  - Accept lower precision (more false alarms) ← Can be verified")
print('='*70)
