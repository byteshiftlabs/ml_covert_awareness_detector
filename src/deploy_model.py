...
def predict_subject(subject, condition=None, model_path=None):
    ...
    for cond_idx in conditions_to_predict:
        ...
        feature_names = [k for k in features.keys() 
                        if k not in ['subject', 'condition', 'label', 'connectivity']]
        X_basic = np.array([[features[name] for name in feature_names]])
        
        # For deployment, use all-condition mean as baseline
        X_deviation = np.zeros_like(X_basic)  # Simplified for single prediction
        ...
        cond_names = ['rest1', 'imagery_awake', 'pre-LOR', 'LOR', 'post-ROR', 'imagery4', 'rest2']
        ...
        if cond_idx == 3:  # LOR
            print(f"  Expected: UNCONSCIOUS (loss of responsiveness)")
        elif cond_idx in CONSCIOUS_CONDITIONS:
            print(f"  Expected: CONSCIOUS")

def main():
    parser = argparse.ArgumentParser(description='Consciousness detection model deployment')
    parser.add_argument('--subject', type=str, default='sub-02', help='Subject ID')
    parser.add_argument('--condition', type=int, default=None, help='Condition (0-6), or all if not specified')
    parser.add_argument('--train', action='store_true', help='Train and save final model')
